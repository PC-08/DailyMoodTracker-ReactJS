...
class App extends Component {
  state = {
    activeMonth: 1,
    activeEmoji: emojisList[0],
    monthList: initialMonthsList,
    selectEmojiCount: 0,
    selectedEmoji: emojisList[0].emojiName,
    selectedDay: daysList[0].dayNumber,
    homeActive: true,
    reportActive: false,
  }

  onSelectEmoji = id => {
    const selectedEmoji = emojisList.filter(eachEmoji => eachEmoji.id === id)
    console.log(selectedEmoji)
    this.setState({activeEmoji: selectedEmoji[0]}, this.getEmojiCount)
  }

  getEmojiCount = () => {
    const {monthList, selectedEmoji, selectedDay, activeMonth} = this.state

    console.log(monthList, selectedDay, selectedEmoji, activeMonth)

    let count = 0
    const datesList = monthList[activeMonth - 1].dates
    datesList.forEach(eachDate => {
      const dayNum = parseInt(eachDate.date)
      if (
        dayNum % 7 === parseInt(selectedDay) &&
        eachDate.emojiName === selectedEmoji
      ) {
        count += 1
      }
    })
    ...
    this.setState({
      selectEmojiCount: ans,
    })
  }

  updateMonthMethod = monthNum => {
    console.log(monthNum)
    if (monthNum > 12) {
      this.setState({activeMonth: 12}, this.getEmojiCount)
    } else if (monthNum < 1) {
      this.setState({activeMonth: 1}, this.getEmojiCount)
    } else {
      this.setState({activeMonth: monthNum}, this.getEmojiCount)
    }
  }

  onSelectDayChange = event => {
    this.setState({selectedDay: event.target.value}, this.getEmojiCount)
  }

  onSelectEmojiChange = event => {
    this.setState({selectedEmoji: event.target.value}, this.getEmojiCount)
  }

  updateContextMonthListMethod = props => {
    const {activeEmoji, month, dateId} = props
    const {monthList} = this.state

    const datesList = monthList[month - 1].dates
    const dateObj = datesList.find(eachDate => eachDate.id === dateId)

    let updatedObj

    if (dateObj.emojiName === activeEmoji.emojiName) {
      updatedObj = {
        ...dateObj,
        emojiName: '',
        emojiUrl: '',
    ...
    const newDatesList = datesList.map(eachDate =>
      eachDate.id === dateId ? updatedObj : eachDate,
    )

    const newMonth = {...monthList[month - 1], dates: newDatesList}

    const updatedMonthList = [
      ...monthList.slice(0, month - 1),
      newMonth,
      ...monthList.slice(month),
    ]

    console.log(updatedMonthList)

    this.setState({monthList: updatedMonthList}, this.getEmojiCount)
  }

  onClickHomeTabCon = () => {
    this.setState({homeActive: true, reportActive: false})
  }

  onClickReportTabCon = () => {
    this.setState({homeActive: false, reportActive: true})
  }
    ...
    return (
      <MoodTrackerContext.Provider
        value={{
          ...
          homeActive,
          reportActive,
          updateMonthMethod: this.updateMonthMethod,
          updateContextMonthListMethod: this.updateContextMonthListMethod,
          onSelectDayChange: this.onSelectDayChange,
          onSelectEmojiChange: this.onSelectEmojiChange,
          onClickReportTabCon: this.onClickReportTabCon,
          onClickHomeTabCon: this.onClickHomeTabCon,
